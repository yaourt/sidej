...
public class CheckAvailableRedisCommandsIT {
	...
	private static SortedSet<RedisJSONCommand> getAvailableRedisCommands() throws IOException {
		...
		try {
			...
			final Map<String, RedisJSONCommand> commandsMap = mapper.readValue(remoteCommandsStream, new TypeReference<Map<String, RedisJSONCommand>>(){});
	...
	private void generateIRedisClient() throws IOException {
		...
		for(final RedisJSONCommand cmd : commands) {
			...
			strb
				.append("\t */\n")
				.append("\tUpdateMe ").append(cmd.getName()).append('(');
	...
	private List<String> getDocumentation(final RedisJSONCommand command) throws IOException {
		final ArrayList<String> result = new ArrayList<String>(20);
		BufferedReader reader = null;
		try {
			final StringBuilder strb =
					new StringBuilder(DOCUMENTATION_BASE_URL)
					.append(command.getName())
					.append(DOCUMENTATION_SUFFIX);
